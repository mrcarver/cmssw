...
double tools::getMiniIsolation(edm::Handle<pat::PackedCandidateCollection> pfcands,
	...
	if(em){
    	...
    	else if( TMath::Abs( dynamic_cast<const pat::Electron *>(ptcl)->superCluster()->eta() ) > 2.4 && TMath::Abs( dynamic_cast<const pat::Electron *>(ptcl)->superCluster()->eta() ) < 2.5  )   CorrectedTerm = rho * Aeff_Fall15Anal[0][ 6 ]*(riso2/0.09);
    }
...
std::vector<const pat::Electron* > tools::MVALooseElectronSelector(//const std::vector<pat::Electron>  & thePatElectrons,
	...
	for( /*std::vector<pat::Electron>::const_iterator*/ auto el = thePatElectrons->begin() ; el != thePatElectrons->end() ; el++ ) {
   ...
   //std::cout<<"electron pT/eta/phi = "<<el->pt()<<"/"<<el->eta()<<"/"<<el->phi()<<"\n";
   
    ////Do online trigger cuts////
    if( TMath::Abs( el->eta()) < 1.479  ) {
	
		if(el->full5x5_sigmaIetaIeta() > 0.011) continue;
		if(el->hcalOverEcal() > 0.08) continue;
		if(fabs(el->deltaEtaSuperClusterTrackAtVtx()) > 0.01) continue;
		if(fabs(el->deltaPhiSuperClusterTrackAtVtx()) > 0.04) continue;
		//std::cout<<"1/E - 1/p = "<<(1.0/el->correctedEcalEnergy()) -    el->eSuperClusterOverP()/el->correctedEcalEnergy()<<"\n";
		if( fabs( (1.0/el->correctedEcalEnergy()) -    el->eSuperClusterOverP()/el->correctedEcalEnergy()  )  > 0.01) continue;
	
	}
	else{
	
		if(el->full5x5_sigmaIetaIeta() > 0.031) continue;
		if(el->hcalOverEcal() > 0.08) continue;
		if(fabs(el->deltaEtaSuperClusterTrackAtVtx()) > 0.01) continue;
		if(fabs(el->deltaPhiSuperClusterTrackAtVtx()) > 0.08) continue;
		//std::cout<<"1/E - 1/p = "<<(1.0/el->correctedEcalEnergy()) -    el->eSuperClusterOverP()/el->correctedEcalEnergy()<<"\n";
		if( fabs( (1.0/el->correctedEcalEnergy()) -    el->eSuperClusterOverP()/el->correctedEcalEnergy()  )   > 0.01) continue;
    ...
    const edm::Ptr<pat::Electron> elePtr(thePatElectrons,el-thePatElectrons->begin()); //value map is keyed of edm::Ptrs so we need to make one
    float value = valuesMap[elePtr];
	
	double mvaThresh = -10.0;

	...
	cfg=SSSync_Interactive.py
	#cfg=Run2Data_Ntuplizer.py
	count=0
	input=$1
	...
	BASE=/lfs/scratch/mrcarver/Fall15AnalysisCode/CMSSW_7_4_14/src/SUSYAnalyzer/PatAnalyzer/test
	cd \$BASE
	...
	cmsRun ${cfg} input=\"$FILES\" output=${OUTPUT}/Job_${JOB}.root > ${OUTPUT}/logs/Job_${JOB}.txt 2>  ${OUTPUT}/errs/Job_${JOB}.txt
	" > submit.sh ;

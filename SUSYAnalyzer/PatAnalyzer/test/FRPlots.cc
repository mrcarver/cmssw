...
void FRPlots()
{
	...
	TString nameFile = "/cms/data/store/user/t2/users/mrcarver/Run2Tuples/Data/ModBtag_Precaled_FR";

    TString files[3] = {"WJetsAll","ZJetsAll","TTJetsAll"};
   ...
   /*for(int p=0;p<21;p++){
	...
	TFile* file = new TFile("/cms/data/store/user/t2/users/mrcarver/Run2Tuples/Data/ModBtag_Prescaled.root","read");
	...
	//	data->SetBinContent(b+1,data->GetBinContent(b+1)*80);
 	...
 	plots[p] = new TCanvas(PNames[p],PNames[p],2);
	plots[p]->cd();
	...
	*/
	...
	float pbs[6] = {10.0,15.0,20.0,35.0,50.0,70.0};
	...
	plots[p]->SaveAs("PromptPlots_Prescaled/"+PNames[p]+".png");
